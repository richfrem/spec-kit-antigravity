...
*   **`memory/`**: Constitution and agent context templates.
*   **`scripts/`**: Bash/PowerShell scripts (e.g., `check-prerequisites.sh`). Kept in root for easy access by manual users.
*   **`templates/`**: Project file templates (e.g., `spec-template.md`).

### Build Artifacts
These are generated files:

*   **`.agent/`**: A consolidated folder containing all workflows, memory, and templates. **This folder is generated by `build.py`.**
